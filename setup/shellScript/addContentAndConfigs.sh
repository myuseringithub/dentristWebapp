...
if [ "$(ls -A '/app')" ] || [ "$(ls -A '/etc/apache2/sites-available')" ]; then
    ...
    mkdir -p /app/root/assets/; cp -pr /tmp/source/clientSide/assets/ /app/root/assets/
    ...
    mv /app/root/assets /app/root/app
    # wp-content
    ...
    # Bower Endpoint to allow installation from Bower sources
    npm install -g jspm-bower-endpoint
    jspm registry create -y bower jspm-bower-endpoint
    # Install NPM & JSPM packages:
    (cd /tmp/source/clientSide/jspm_packages_modules/; \
    jspm install)
    # move to desired location: 
    mv /tmp/source/clientSide/jspm_packages_modules/jspm_packages/ /app/root/app/sharedApp/javascripts/
    ...
    find /app/ -type f -exec chmod -R 777 {} \;
    find /app/ -type d -exec chmod -R 777 {} \;
